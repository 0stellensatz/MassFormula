...
- `Development.lean` needs `open <Project>` to name the production declarations it delegates to, and only there does the short name become ambiguous between a clone and its original. Write the production one `<Project>.foo` at the call site rather than relying on which side the resolution order favors.
